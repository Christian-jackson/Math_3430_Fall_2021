...
def test_add_vecs():
    assert LA_02.add_vectors(test_vector_01, test_vector_02) == [4, 3, 6]
    assert LA_02.add_vectors(test_vector_01, test_vector_03) == [5, 8, 6]

def test_scalar_vec_multi():
    assert LA_02.scalar_vec_multi(test_vector_01, test_scalar_01) == [2, 4, 8]
    assert LA_02.scalar_vec_multi(test_vector_02, test_scalar_02) == [9, 3, 6]

def test_scalar_matrix_multi():
    assert LA_02.scalar_matrix_multi(test_matrix_01, test_scalar_01) == [[2, 4, 6],[8, 10, 12],[14, 16, 18]]
    assert LA_02.scalar_matrix_multi(test_matrix_02, test_scalar_01) == [[4, 8, 12], [16, 20, 24], [28, 32, 36]]

def test_matrix_add():
    assert LA_02.matrix_add(test_matrix_01, test_matrix_02) == [[3, 6, 9],[12, 15, 18],[21, 24, 27]]
    assert LA_02.matrix_add(test_matrix_01, test_matrix_03) == [[2, 5, 8],[11, 14, 17],[20, 23, 26]]

def test_mat_vec_multi():
    assert LA_02.mat_vec_multi(test_matrix_01, test_vector_02) == [21, 27, 33]
    assert LA_02.mat_vec_multi(test_matrix_02, test_vector_03) == [84, 108, 132]

def test_mat_mat_multi():
    assert LA_02.mat_mat_multi(test_matrix_01, test_matrix_02) == [[60, 72, 84], [132, 162, 192], [204, 252, 300]]
    assert LA_02.mat_mat_multi(test_matrix_01, test_matrix_03) == [[48, 57, 66], [120, 147, 174], [192, 237, 282]]